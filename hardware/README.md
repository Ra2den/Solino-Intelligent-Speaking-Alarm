...
This directory contains everything that runs on the Raspberry Pi and the Seeed XIAO nRF52840 sensor node. The Pi runs a Node.js control server that drives the LED strip and screen brightness. It polls the PC backend every second for the current alarm state and reacts accordingly. Bed-presence data arrives from the XIAO over BLE. Alarm audio plays on the PC as normal — no audio setup on the Pi is required for the alarm.
...
The hardware folder lives on the `deploy-on-hardware` branch of the main repo:

```bash
git clone https://github.com/Ra2den/Solino-Intelligent-Speaking-Alarm.git ~/wecker
cd ~/wecker
git checkout deploy-on-hardware
cd hardware/src
npm install
```

If you previously installed a separate control server, stop and remove it first:

```bash
sudo systemctl stop wecker-control wecker-ble
sudo systemctl disable wecker-control wecker-ble
```
...
No changes are needed on the PC side. The Pi does all the work.

### On the Pi (control dashboard)

Set **PC Backend URL** in the Settings card of the control dashboard (`http://<pi-ip>:5000`), e.g. `http://192.168.1.42:8000`. The value persists to `hardware/settings.json` across restarts.

With this set, the Pi will:
- Poll `GET /alarm-session/current` every second and drive LED strip and screen brightness based on the alarm state
- Forward bed-sensor events to `POST /alarm-session/{id}/pressure` using the session ID from the last poll

The alarm sound continues to play on the PC as normal. No `PI_CONTROL_URL` or any other setting is needed in the backend `.env`.
...
|--------|------|------|-------------|
| `GET` | `/api/state` | — | Full current state (LEDs, screen, bed sensor, alarm status) |
| `POST` | `/api/alarm/state` | `{status}` | Manually set alarm state — drives LED and screen (also updated automatically by polling) |
...
**LEDs/screen not reacting to alarm state**
Check the Pi's Node server logs for `[Poll]` lines — they appear every second when `PC_BACKEND_URL` is set and the backend is reachable. Common causes:
- `PC_BACKEND_URL` not set in the dashboard Settings
- PC firewall blocking port 8000
- Wrong IP or the PC backend not running

**Bed sensor events not reaching the PC alarm service**
Check the Pi's Node server logs for `[Sensor]` and `[PC push]` lines. The Pi uses the session ID from the last poll, so if the poll is failing the sensor push will also fail. Fix the poll first (see above).
