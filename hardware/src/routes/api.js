...
// Poll the PC backend for current alarm session state every second.
// Updates LEDs/screen and stores the current session ID for sensor forwarding.
let _lastPolledStatus = null;
let _currentSessionId = null;

function _pollBackend(applyAlarmState) {
  if (!config.PC_BACKEND_URL) return;
  const url = new URL("/alarm-session/current", config.PC_BACKEND_URL);
  http.get(url.toString(), (res) => {
    ...
    res.on("end", () => {
      try {
        ...
        const newStatus = session ? session.status : "IDLE";
        _currentSessionId = session ? session.id : null;
        if (newStatus !== _lastPolledStatus) {
          console.log(`[Poll] alarm status: ${_lastPolledStatus} → ${newStatus}`);
          _lastPolledStatus = newStatus;
          applyAlarmState(newStatus);
        }
      } catch (e) {
        console.warn("[Poll] Parse error:", e.message);
      }
    });
  }).on("error", (e) => {
    console.warn("[Poll] Backend unreachable:", e.message);
  });
}

function _startPolling(applyAlarmState) {
  setInterval(() => _pollBackend(applyAlarmState), 1000);
}
...
module.exports = function apiRouter(io) {
  ...
  _startPolling(_applyAlarmState);
  ...
  router.post("/sensor/data", (req, res) => {
    ...
    if (changed) {
      if (_currentSessionId !== null) {
        console.log(`[Sensor] Pushing to PC: session=${_currentSessionId} is_pressed=${occupied}`);
        _pushToPc(`/alarm-session/${_currentSessionId}/pressure`, { is_pressed: occupied });
      } else {
        console.log("[Sensor] No active session — skipping pressure push");
      }
    }
