...
export function AlarmCard({
  ...
  return (
    ...
    >
      ...
      {!isWidget && (
        <>
          {/* Left Action: Delete (visible when translating to the right, i.e. > 0) */}
          <div
            style={{ width: `${Math.max(0, translateX)}px` }}
            className="absolute left-0 top-0 bottom-0 overflow-hidden bg-rose-400 text-white rounded-[50px] flex items-center"
          >
          ...
          {/* Right Action: Edit (visible when translating to the left, i.e. < 0) */}
          <div
            style={{ width: `${Math.max(0, -translateX)}px` }}
            className="absolute right-0 top-0 bottom-0 overflow-hidden bg-indigo-400 text-white rounded-[50px] flex items-center justify-end"
          >
